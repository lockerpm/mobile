...
class CallerIDManager: NSObject {
  ...
  func isExtensionActived(_ resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) {
    ...
    if #available(iOS 18.2, *) {
      if LiveCallerIDLookupManager.shared.status(forExtensionWithIdentifier: "com.cystack.lockerapp.LockerLiveCallerLookup") == .enabled {
        resolve(true)
        return
      }
    }
  ...
  func reset(_ resolve: @escaping RCTPromiseResolveBlock,
      ...
      if #available(iOS 18.2, *) {
          Task {
              do {
                try await LiveCallerIDLookupManager.shared.reset(forExtensionWithIdentifier: "com.cystack.lockerapp.LockerLiveCallerLookup")
                resolve(nil)
              } catch {
  ...
  func refreshPIRParameters(_ resolve: @escaping RCTPromiseResolveBlock,
      ...
      if #available(iOS 18.2, *) {
          Task {
              do {
                try await LiveCallerIDLookupManager.shared.refreshPIRParameters(forExtensionWithIdentifier: "com.cystack.lockerapp.LockerLiveCallerLookup")
                resolve(nil)
              } catch {
