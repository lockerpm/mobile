...
export class CipherService implements CipherServiceAbstraction {
  ...
  async encrypt(
    ...
    if (model.id != null) {
      ...
      if (originalCipher != null) {
        ...
        if (
          (model.type === CipherType.Login && existingCipher.type === CipherType.Login) ||
          (model.type === CipherType.MasterPassword &&
            existingCipher.type === CipherType.MasterPassword)
        ) {
          if (
  ...
  private async batchDecrypt(page: number): Promise<CipherView[]> {
    ...
    const start =
      page === 0
        ? 0
        : (page - 1) * BACKGROUND_DECRYPT_BATCH_SIZE + BACKGROUND_DECRYPT_FIRST_BATCH_SIZE
    const expectedEnd =
      page === 0
        ? BACKGROUND_DECRYPT_FIRST_BATCH_SIZE
        : page * BACKGROUND_DECRYPT_BATCH_SIZE + BACKGROUND_DECRYPT_FIRST_BATCH_SIZE
  ...
  async getAllDecryptedForUrl(
    ...
    return ciphers.filter((cipher) => {
      ...
      if (
        url != null &&
        (cipher.type === CipherType.Login || cipher.type === CipherType.MasterPassword) &&
  ...
  getLocaleSortingFunction(): (a: CipherView, b: CipherView) => number {
    return (a, b) => {
      ...
      if (
        result !== 0 ||
        ((a.type !== CipherType.Login || b.type !== CipherType.Login) &&
          (a.type !== CipherType.MasterPassword || b.type !== CipherType.MasterPassword))
      ) {
        return result
      }
  ...
  private async getCipherForUrl(
    ...
    if (!this.sortedCiphersCache.isCached(cacheKey)) {
      ...
      if (autofillOnPageLoad) {
        const autofillOnPageLoadDefault = await this.storageService.get(
