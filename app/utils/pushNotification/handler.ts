...
import { AppStorageKey, loadString } from "../storage"
...
export const handleNewShare = async (data: string | object) => {
  ...
  const { userId, isVn } = _getCurrentUser()

  // Only noti current user
  if (!userId || !shareData.pwd_user_ids.map((i) => i.toString()).includes(userId)) {
    return
  }
...
export const handleConfirmShare = async (data: string | object) => {
  ...
  // Only noti current user
  if (!shareData.pwd_user_ids.map((i) => i.toString()).includes(userId)) {
    return
  }
...
export const handleTipTrick = async (data: string | object) => {
  ...
  const { language } = _getCurrentUser()
  const supportLang = language === "vi" ? "vi" : "en"
  const text = tipTrickdata.data.title[supportLang]
  ...
  notify({
    ...
    data: {
      ...
      url: tipTrickdata.data.metadata.link[supportLang],
    },
  })
}
// ------------------ PRIVATE --------------------
...
const _getCurrentUser = () => {
  const language = loadString(AppStorageKey.LAST_USED_LOCALE) ?? "vi"
  const userId = loadString(AppStorageKey.APP_CURRENT_USER_PW_ID) ?? ""

  return { isVn: language === "vi", userId, language }
}
