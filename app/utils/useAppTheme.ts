...
import { AppStorageKey, loadString, saveString } from "./storage"
...
export const useThemeProvider = () => {
  const initialTheme = loadString(AppStorageKey.LAST_USED_THEME) || undefined

  const colorScheme = useColorScheme()
  const [overrideTheme, setTheme] = useState<ThemeContexts>(initialTheme as ThemeContexts)
  ...
  saveString(AppStorageKey.LAST_USED_THEME, themeScheme)
...
interface UseAppThemeValue {
  ...
  setThemeContext: (newTheme: ThemeContexts) => void
...
export const useAppTheme = (): UseAppThemeValue => {
  ...
  return {
    navTheme,
    setThemeContext: setThemeContextOverride,
    theme: themeVariant,
    themeContext,
    themed,
  }
