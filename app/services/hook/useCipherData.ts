...
export function useCipherData() {
  ...
  const startSyncProcess = (bumpTimestamp: number) => {
    ...
    return syncQueue.add(async () => {
      try {
        ...
        cipherStore.setLastCacheUpdate()
        // Sync other ciphers
        const totalCipherCount = res.data.count?.ciphers ?? 0
      ...
      const folders = await Promise.all(
        res.map(async (folder) => {
          ...
          const ciphers = await getEncryptedCiphers({
            ...
            filters: [
              (c: CipherView) =>
                (c.collectionIds.length === 0 && c.folderId && c.folderId === folder.id) || false,
            ],
          })
          return {
            ...folder,
  ...
  const getEncKeyFromDecryptedKey = async (encryptKey: string) => {
    const keyBuffer = await cryptoService.rsaDecrypt(encryptKey)
  ...
  const importCiphers = async (payload) => {
    ...
    if (isFreeAccount) {
      ...
      let currentItemsCount = await _getCipherCount([
        CipherType.Login,
        CipherType.Card,
        CipherType.Identity,
      ...
      for (let i = 0; i < importResult.ciphers.length; i++) {
        ...
        if (currentItemsCount < FREE_PLAN_LIMIT.ITEMS) {
          currentItemsCount += 1
        } else {
          setIsLimited(true)
          break
  ...
  const createCipher = async (
    ...
    try {
      ...
      if (res.kind === "ok") {
        ...
        if (!silent) {
          notifyTx("success", "success:cipher_created")
        }
      } else {
        notifyApiError(res)
      }
  ...
  const updateCipher = async (
    ...
    try {
      ...
      if (res.kind === "ok") {
        ...
        if (!silent) {
          notifyTx("success", "success:cipher_updated")
        }
      } else {
        notifyApiError(res)
      }
