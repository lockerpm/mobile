...
const defaultData = {
  ...
  shareFolderCreateItem: async (collection: CollectionView) => {
    return { kind: 'unknown' }
  },
  shareFolderAddItem: async (collection: CollectionView, cipher: CipherView) => {
    return { kind: 'unknown' }
  },
  shareFolderAddMultipleItems: async (collection: CollectionView, cipherIds: string[]) => {
    return { kind: 'unknown' }
  },
  shareFolderRemoveItem: async (collection: CollectionView, cipher: CipherView) => {
    return { kind: 'unknown' }
  },
  stopShareFolder: async (collection: CollectionView) => {
    return { kind: 'unknown' }
  },
}
...
export const FolderMixinsProvider = observer(
  (props: { children: boolean | React.ReactChild | React.ReactFragment | React.ReactPortal }) => {
    ...
    return (
      <FolderMixinsContext.Provider value={data}>{props.children}</FolderMixinsContext.Provider>
    )
  }
)
