...
const defaultData = {
  ...
  updateNewMasterPasswordEA: async (newPassword: string, email: string, eaID: string) => { return { kind: 'unknown' } },
...
export const CipherAuthenticationMixinsProvider = observer((props: { children: boolean | React.ReactChild | React.ReactFragment | React.ReactPortal }) => {
  ...
  const updateNewMasterPasswordEA = async (newPassword: string, email: string, eaID: string): Promise<{ kind: string }> => {
    try {
