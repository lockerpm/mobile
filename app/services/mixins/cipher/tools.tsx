...
const defaultData = {
  ...
  getCipherCount: async (type: CipherType, deleted?: boolean, share?: boolean): Promise<number> => {
    return 0
  },
  checkLoginIdExist: async (id: string): Promise<boolean> => {
    return false
  },
}
...
export const CipherToolsMixinsProvider = observer(
    ...
    const loadPasswordsHealth = async () => {
      try {
        ...
        weakPasswordCiphers.sort((a, b) => {
          return (
            passwordStrengthCache.get(getCacheKey(a)) - passwordStrengthCache.get(getCacheKey(b))
          )
        })
