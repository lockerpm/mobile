...
import { Screen, Text, TabHeader } from 'app/components-v2/cores'
import { MenuItem, MenuItemContainer, MenuItemProps } from "app/components-v2/utils"
...
export const MenuScreen = () => {
  ...
  const item3 = {
    ...
    "pm_premium": {
      node: <View style={{ flexDirection: isSmallWidth ? "column" : "row" }}>
        ...
        <Text text={translate("menu.expired_time") + ": " + moment(user.plan?.next_billing_time * 1000).format("DD MMMM YYYY")} style={[$planName, { marginLeft: isSmallWidth ? 0 : 8 }]} />
      </View>
    },
    "pm_family": {
      node: <View style={{ flexDirection: isSmallWidth ? "column" : "row" }}>
  ...
  return (
    <Screen
      ...
      header={(
        <TabHeader title={translate('common.menu')} />
      )}
    >
      ...
      {
        user.pwd_user_type !== "enterprise" && <ReferFriendMenuItem onPress={() => navigation.navigate('refer_friend', {
          referLink: referLink
        })} />
      }
      ...
      <View style={{
        ...
        <Text preset="label" weight="semibold">{'CyStack'}</Text>
      </View>
      <Text preset="label" style={{ marginVertical: 8, textAlign: 'center' }}>Locker - {appVersion}</Text>
    </Screen>
  )
}
