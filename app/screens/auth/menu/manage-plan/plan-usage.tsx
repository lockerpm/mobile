...
const ItemStorage = (props: PlanStorageProps) => {
    const { cipherType, style, limits, title } = props
    ...
    return (
        <View style={[{ width: '100%', marginVertical: 4 }, style]}>
            <View style={{
                ...
                <Text preset="black"> {cipherCount}/{limits}</Text>
            </View>
            ...
            <ProgressBar
                ...
                // @ts-ignore
                progress={cipherCount / limits * 100}
            />
        </View>
    )
}
    ...
    return (
        <View style={[commonStyles.SECTION_PADDING, { backgroundColor: color.background, marginBottom: 10 }]}>
            ...
            <View style={USAGE_CONTAINER}>
                <Text preset="default" text="Free Plan Usage" style={{ marginBottom: 8 }} />
                {
                    items.map(
                        e => <ItemStorage key={e.cipherType} cipherType={e.cipherType} limits={e.limits} title={e.title} />
                    )
                }    
            </View>
            <Button
                onPress={() => { navigation.navigate("payment") }}
                text="Increase unlimited with Premium"
                style={{ marginTop: 12 }}
            />
        </View>
    )
