...
export const BusinessPasswordlessQrScan = observer(
  ({ otp, goBack, index }: Props) => {

    const {translate} = useMixins()
    const { width, height } = Dimensions.get("screen")
    const navigation = useNavigation()
    const [onScanQR, setonScanQR] = useState(false)
    const { sessionBusinessQrLogin } = useCipherAuthenticationMixins()

    const onSuccess = async (e) => {
      const res = await sessionBusinessQrLogin(
        e.data,
        otp.toString(),
      )
