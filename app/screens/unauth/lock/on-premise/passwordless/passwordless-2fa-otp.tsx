...
export const OnPremiseOtp = observer((props: Props) => {
  ...
  const { sessionOtpLoginWithHashPassword } = useCipherAuthenticationMixins()
  const { goBack, method, email, pwdHash, symmetricCryptoKey} = props
  ...
  const handleAuthenticate = async () => {
    setIsError(false)
    setIsLoading(true)
    const res = await sessionOtpLoginWithHashPassword(pwdHash, symmetricCryptoKey, method, otp, saveDevice)
    setIsLoading(false)
    if (res.kind === "ok") {
