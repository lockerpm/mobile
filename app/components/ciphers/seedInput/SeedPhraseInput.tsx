...
export const SeedPhraseInput = (props: Props) => {
  ...
  const handleChange = (val: string, index: number) => {
    ...
    if (ws.length > 1 && index + ws.length - 1 < MAX_WORD_COUNT) {
      ...
      setTimeout(() => {
        refs.current[index + ws.length - 1]?.focus()
      }, 0)
  ...
  return (
    ...
    >
      ...
      <TextInput
        ...
        style={[$input, { color: colors.title }]}
      />
      {disableEdit && <TouchableOpacity style={$copy} onPress={() => copyToClipboard(value)} />}
    </View>
  )
}
