...
export function getActiveRouteName(state: NavigationState | PartialState<NavigationState>): string {
  ...
  if (!route.state) return route.name as keyof AppRoute

  // Recursive call to deal with nested routers
  return getActiveRouteName(route.state as NavigationState<AppRoute>)
}
...
export function useNavigationPersistence() {
  ...
  const onNavigationStateChange = (state: NavigationState | undefined) => {
    ...
    if (state !== undefined) {
      ...
      routeNameRef.current = currentRouteName as keyof AppRoute
...
export const modalScreenOptions: NativeStackNavigationOptions = {
  ...
  animation: "fade",
  contentStyle: {
    backgroundColor: "rgba(25, 16, 21, 0.6)",
  },
}
