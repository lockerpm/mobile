...
import { iconRegistry, iconRegularRegistry, IconTypes } from '../icon/Icon'
...
function Checkbox(props: ToggleInputProps) {
  ...
  const offBackgroundColor = [
    disabled && colors.palette.neutral4,
    status === 'error' && colors.error,
    colors.palette.neutral2,
  ].filter(Boolean)[0]

  const outerBorderColor = [
    disabled && colors.palette.neutral4,
    status === 'error' && colors.error,
    !on && colors.palette.neutral8,
    colors.border,
  ].filter(Boolean)[0]
  ...
  const iconTintColor = [
    disabled && colors.palette.neutral6,
    status === 'error' && colors.error,
    colors.border,
  ].filter(Boolean)[0]
  ...
  return (
    ...
    >
      ...
      >
        <Image
          source={iconRegistry[checkboxIcon] || iconRegularRegistry.check}
...
function Switch(props: ToggleInputProps) {
  ...
  // @ts-ignore
  const knobBackgroundColor: ColorValue = (function () {
    if (on) {
      return [
...
function SwitchAccessibilityLabel(props: ToggleInputProps & { role: 'on' | 'off' }) {
  ...
  const color = (function () {
    ...
    if (!on) return innerStyle?.backgroundColor || colors.border
    return detailStyle?.backgroundColor || colors.palette.neutral1
  })()
