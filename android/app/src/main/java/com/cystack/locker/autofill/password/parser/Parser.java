...
//        }
        ...
        if (!PasswordUtils.isNullOrWhiteSpace(packageName)){
            result.packageName = packageName;
        }

        if (!PasswordUtils.isNullOrWhiteSpace(domain)){
            result.domain = domain;
        }
        if (!PasswordUtils.isNullOrWhiteSpace(uri)){
            result.uri = uri;
        }
    ...
    {
        if (PasswordUtils.isNullOrWhiteSpace(packageName) && !PasswordUtils.isNullOrWhiteSpace(node.getIdPackage())) {
            packageName = node.getIdPackage();
        }
        if (PasswordUtils.isNullOrWhiteSpace(domain) && !PasswordUtils.isNullOrWhiteSpace(node.getWebDomain())) {
            String scheme = "http";
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                scheme = node.getWebScheme();
            }

            if (!PasswordUtils.isNullOrWhiteSpace(node.getWebDomain())){
    ...
    {
        if (node != null && !PasswordUtils.isNullOrWhiteSpace((String) node.getTitle()))
        {
    ...
    public static class Result {
        ...
        public String getUri() {
            if (!PasswordUtils.isNullOrWhiteSpace(uri)) {
                return uri;
            } else if (!PasswordUtils.isNullOrWhiteSpace(packageName)){
        ...
        public String getDomain() {
            return !PasswordUtils.isNullOrWhiteSpace(domain) ? domain : packageName;
        }
        @Nullable
