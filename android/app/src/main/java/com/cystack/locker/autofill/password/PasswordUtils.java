...
public class PasswordUtils {
    ...
    public static void openChromeDeepLinkSettings(Context context) throws Exception {
        ...
        autofillSettingsIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
    ...
    private static String pbkdf2(String password, String salt, int iterations, int keyLength)
    ...
    public static FillResponse.Builder BuildFillResponse(ArrayList<Field> fields, @NonNull FillRequest request,
            String domain, Context context) {
        ...
        if (storedItem != null) {
            response.addDataset(
                    BuildDataSetWithAuthen(fields, storedItem, itemInlinePresentationSpec, domain, context));
        }
    ...
    public static Dataset BuildDataSetLocker(ArrayList<Field> fields, InlinePresentationSpec inlinePresentationSpec,
            String domain, Context context) {
        RemoteViews presentation = new RemoteViews(context.getPackageName(), R.layout.remote_locker_app);

        PendingIntent authentication = RNAutofillServiceAndroid.intentForFillPassword(context, fields, domain);

        return BuildlockedDataset(fields, null, presentation, authentication, inlinePresentationSpec, context);
    }

    public static Dataset BuildDataSetWithAuthen(ArrayList<Field> fields, AutofillItem data,
            InlinePresentationSpec inlinePresentationSpec, String domain, Context context) {
        ...
        PendingIntent authentication = RNAutofillServiceAndroid.intentForQuickBarPassword(context, fields, domain,
                data.getId());
    ...
    static Dataset BuildlockedDataset(@NonNull ArrayList<Field> fields, AutofillItem data,
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            InlinePresentation inlinePresentation = buildInlinePresentation(inlinePresentationSpec, authentication,
                    data, context);
    ...
    private static InlinePresentation buildInlinePresentation(InlinePresentationSpec inlinePresentationSpec,
            PendingIntent pendingIntent, AutofillItem data, Context context) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.R) {
            return null;
        }
    ...
    public static void AddSaveInfo(@NonNull FillRequest fillRequest, FillResponse.Builder response, List<Field> fields,
            String packageName) {
        // Docs state that password fields cannot be reliably saved in Compat mode since
        // they will show as
        // masked values.
        boolean compatRequest = false;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            // Attempt to automatically establish compat request mode on Android 10+
            compatRequest = (fillRequest.getFlags() | FillRequest.FLAG_COMPATIBILITY_MODE_REQUEST) == fillRequest
                    .getFlags();
        }
